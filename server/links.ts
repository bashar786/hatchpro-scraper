  ...
  try {
    ...
    const productData = await page.evaluate(() => {
      ...
      return {
        ...
        price: extractTextByClass('.product-price__value'),
        availability: availability,
        estimateAvailability: estimateAvailability,
        description: extractTextByClass('.product-side-rail__description'),
        images: Array.from(document.querySelectorAll('img')).map(img => img.src).join('; '),
        links: Array.from(document.querySelectorAll('a')).map(anchor => anchor.href).join('; ')
      };
    });
